...
void getnew(){
	...
	/*else if (sums < (50 * tol) && wallDist > 600 && seenCar){
		isParkSpace = true;
		myfile << "Found Spot! car dist:" << carDist << "next car dist:" << nextCarDist << endl;
	}
	else if (sums < (50 * tol) && wallDist == 0 && seenCar){
		isParkSpace = true;
		myfile << "Found Spot! car dist:" << carDist << "next car dist:" << nextCarDist << endl;
	}*/
	nineties.clear();
...
//  connector.setupLaser(&sick);
 ...
 // }
  ...
  while(1){
	...
	/*	myfile << ((total.tv_sec+total.tv_usec/float(1000000)) > ((double(carDist) / 300.0) - 1) && !carPassed);
		if ((total.tv_sec+total.tv_usec/float(1000000)) > ((double(carDist) / 300.0) - 1) && !carPassed){
			carNearPassed = true;
		} */
	}
